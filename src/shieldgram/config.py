    ...
    def from_dict(cls, data: dict[str, Any]) -> ShieldConfig:
        ...
        return cls(
            ...
            rate_limiter=RateLimiterConfig(
                ...
                token_bucket_rate=float(rl_data.get("token_bucket_rate", 5.0)),
                token_bucket_burst=int(rl_data.get("token_bucket_burst", 10)),
                enabled=bool(rl_data.get("enabled", True)),
            ),
            flood_detector=FloodDetectorConfig(
                burst_window_seconds=float(fd_data.get("burst_window_seconds", 5.0)),
                burst_threshold=int(fd_data.get("burst_threshold", 10)),
                repeat_window_seconds=float(fd_data.get("repeat_window_seconds", 30.0)),
